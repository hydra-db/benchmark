...
use slatedb_graph_kernel::{
    ...
    GraphStorageMemoryConfig, QueryContext, QueryOutput, RowQueryPlan,
};
...
enum BenchMode {
    ...
    BuildOnly,
    QueryOnly,
    /// Build the graph, then measure Cypher writes against it. Writes emit a
    /// different CSV from reads, so they are a mode of their own rather than
    /// another workload inside the read sweep.
    Writes,
}

impl BenchMode {
    fn builds(self) -> bool {
        matches!(self, Self::Full | Self::BuildOnly | Self::Writes)
    }

    fn writes(self) -> bool {
        matches!(self, Self::Writes)
    }
...
async fn async_main() -> BenchResult<()> {
    ...
    let write_ops = env_usize("GRAPH_QUERY_BENCH_WRITE_OPS", 200);
    let write_concurrency = parse_usize_list(
        "GRAPH_QUERY_BENCH_WRITE_CONCURRENCY",
        &[1, 4, 8, 32],
    );
    let write_workloads = parse_write_workloads(&string_env(
        "GRAPH_QUERY_BENCH_WRITE_WORKLOADS",
        "create,merge,delete",
    ))?;
    ...
    if bench_mode.writes() {
        println!(
            "kind,object_backend,fanout,edges,workload,concurrency,ops,build_ms,build_rss_mib,\
p50_us,p95_us,p99_us,mean_us,ops_per_sec,rss_mib,created_edges,noops,deleted_edges,errors"
        );
        io::stdout().flush()?;
    } else {
    println!(
...
/// Write workloads. All three go through the same Cypher entry point the read
/// benchmark uses, so a write number and a read number are measured the same
/// way and can sit in the same table.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum WriteWorkload {
    /// A brand new edge every time.
    Create,
    /// An edge that is already present. This is the idempotent upsert path a
    /// retrying client takes, and it still pays a commit even when it changes
    /// nothing.
    Merge,
    /// Remove an edge that exists. Each op deletes one edge created untimed
    /// beforehand, so the delete itself is what is measured.
    Delete,
}
...
impl WriteWorkload {
    ...
    /// Cypher for op `n`. Ids are disjoint per workload so the workloads cannot
    /// collide with each other or with the graph the build loaded.
    fn statement(self, base: u64, n: u64) -> String {
        let src = base + n;
        let dst = base + n + 500_000;
        match self {
            Self::Create => {
                format!("CREATE (a {{id: {src}}})-[:{EDGE_TYPE}]->(b {{id: {dst}}})")
            }
            Self::Merge => {
                format!("MERGE (a {{id: {src}}})-[:{EDGE_TYPE}]->(b {{id: {dst}}})")
            }
            Self::Delete => format!(
                "MATCH (a {{id: {src}}})-[e:{EDGE_TYPE}]->(b {{id: {dst}}}) DELETE e"
            ),
        }
    }

    /// Statement that puts the edge in place before a timed op, for the
    /// workloads that need one to exist.
    fn setup(self, base: u64, n: u64) -> Option<String> {
        match self {
            Self::Create => None,
            Self::Merge | Self::Delete => {
                ...
                let dst = base + n + 500_000;
                Some(format!(
                    "CREATE (a {{id: {src}}})-[:{EDGE_TYPE}]->(b {{id: {dst}}})"
                ))
            }
        }
    }
...
/// One shard handle shared by every worker. A second handle would take the
/// writer lease away from the first, so concurrency here means concurrent
/// callers of one writer, which is what an application does.
async fn bench_write_workload(
    shard: &Arc<GraphShard>,
    ...
    id_base: u64,
    tag: &str,
) -> BenchResult<WriteStats> {
    let workers = concurrency.max(1);
    let per_worker = ops.div_ceil(workers);

    // Anything the timed op depends on is created first and left out of the
    // measurement.
    for n in 0..(per_worker * workers) as u64 {
        if let Some(setup) = workload.setup(id_base, n) {
            shard
                .execute_cypher(
                    QueryContext::new(CELL_ID, format!("{tag}-setup-{n}")),
                    &setup,
                )
                .await?;
        }
    ...
    for w in 0..workers {
        ...
        handles.push(tokio::spawn(async move {
            let mut out: Vec<(Duration, Option<QueryMutationCounts>)> =
                Vec::with_capacity(per_worker);
            for i in 0..per_worker {
                let n = (w * per_worker + i) as u64;
                let statement = workload.statement(id_base, n);
                let op_started = Instant::now();
                let result = shard
                    .execute_cypher(
                        QueryContext::new(CELL_ID, format!("{tag}-w{w}-{i}")),
                        &statement,
                    )
                    .await;
                let elapsed = op_started.elapsed();
                out.push((elapsed, result.ok().map(mutation_counts)));
            }
            out
        }));
    }

    let mut latencies = Vec::with_capacity(per_worker * workers);
    let (mut created, mut noops, mut deleted, mut errors) = (0_u64, 0_u64, 0_u64, 0_usize);
    for handle in handles {
        for (elapsed, counts) in handle.await? {
    ...
    Ok(WriteStats {
        stats: LatencyStats::from_durations(&latencies),
        ops_per_sec: (latencies.len() as f64) / elapsed.as_secs_f64().max(f64::MIN_POSITIVE),
        ops: latencies.len(),
        created,
        noops,
        deleted,
...
struct QueryMutationCounts {
    created: u64,
    noops: u64,
    deleted: u64,
}

fn mutation_counts(output: QueryOutput) -> QueryMutationCounts {
    match output {
        QueryOutput::Mutation(result) => QueryMutationCounts {
            created: result.created_edges,
            noops: result.noops,
            deleted: result.deleted_edges,
        },
        _ => QueryMutationCounts {
            created: 0,
            noops: 0,
            deleted: 0,
        },
...
async fn run_write_bench(
    ...
    let shard = Arc::new(
        GraphShard::open_standalone_writer_with_memory_options(
            env.shard_path.clone(),
            Arc::clone(&env.object_store),
            graph_options(None, env.cache_bytes, env.fanout, env.max_hop, env.index_policy),
            graph_memory_config(),
        )
        .await?,
    );

    // The first write on a fresh handle pays for lease acquisition and WAL
    // setup. Left untimed so it does not land in one workload's percentiles.
    shard
        .execute_cypher(
            QueryContext::new(CELL_ID, "write-warmup"),
            &format!("CREATE (a {{id: 900000001}})-[:{EDGE_TYPE}]->(b {{id: 900000002}})"),
        )
        .await?;

    for (wi, workload) in workloads.iter().enumerate() {
        for (ci, &conc) in concurrency.iter().enumerate() {
            // Disjoint id space per (workload, concurrency) so no config can
            // see an edge another one left behind.
            let id_base = 1_000_000_000 + ((wi * 64 + ci) as u64 * 10_000_000);
            let tag = format!("{}-c{conc}", workload.label());
            let result =
                bench_write_workload(&shard, *workload, conc, ops, id_base, &tag).await?;
            println!(
                "{},{},{},{},{},{},{},{},{:.3},{:.3},{:.3},{:.3},{:.3},{:.2},{:.3},{},{},{},{}",
                "write",
                object_backend,
                env.fanout,
                layered_edge_count(env.fanout, env.max_hop),
                workload.label(),
                conc,
                result.ops,
                millis(build_elapsed),
            ...
            io::stdout().flush()?;
            eprintln!(
                "fanout={} workload={} concurrency={conc} ops={} p50_us={:.0} ops_per_sec={:.1} errors={}",
                env.fanout,
                workload.label(),
                result.ops,
                result.stats.p50_us,
...
fn parse_write_workloads(value: &str) -> BenchResult<Vec<WriteWorkload>> {
    let mut out = Vec::new();
    for part in value.split(',').map(str::trim).filter(|p| !p.is_empty()) {
        out.push(match part.to_ascii_lowercase().as_str() {
            "create" => WriteWorkload::Create,
            "merge" => WriteWorkload::Merge,
            "delete" => WriteWorkload::Delete,
            other => {
                return Err(format!("unsupported write workload {other}").into());
            }
        });
    }
    if out.is_empty() {
        return Err("GRAPH_QUERY_BENCH_WRITE_WORKLOADS is empty".into());
    }
    Ok(out)
}
...
fn parse_bench_mode(value: &str) -> BenchResult<BenchMode> {
    match value.trim().to_ascii_lowercase().as_str() {
        ...
        "writes" | "write" | "write-only" | "writeonly" => Ok(BenchMode::Writes),
...
/// macOS has no /proc, and this crate has no libc dependency to call
/// task_info through, so resident size comes from ps. It is only sampled at
/// stage boundaries and after a workload, not per operation, so the fork cost
/// does not land in any measurement. Without this every memory figure on a Mac
